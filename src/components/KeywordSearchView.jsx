...
export default function KeywordSearchView() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="border-b border-zinc-800 pb-6 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
        ...
        <form onSubmit={handleSearchSubmit} className="flex gap-2 w-full md:w-96">
          ...
          <button 
            type="submit"
            disabled={loading}
            className="bg-blue-500 hover:bg-blue-600 disabled:bg-blue-800 text-white rounded-lg px-5 font-semibold text-sm transition-colors shadow-sm cursor-pointer flex items-center justify-center gap-1.5"
          >
            {loading ? (
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center space-y-6">
          <HelpCircle className="text-zinc-700 animate-pulse" size={64} />
          <div>
            <h2 className="text-xl font-bold text-zinc-300">트렌드 키워드를 검색해보세요</h2>
            <p className="text-zinc-500 text-sm mt-1 max-w-sm">유튜브 상의 실시간 검색어 성과 지표와 분야별 분포도를 대시보드로 시각화해 줍니다.</p>
          </div>
          
          <div className="space-y-2.5">
            <span className="text-xs font-semibold text-zinc-500 uppercase tracking-wider block">추천 키워드 클릭</span>
