...
import { Users, Eye, Video, Hash, Image as ImageIcon, ChevronRight, BarChart2, Star, Network } from 'lucide-react';
...
export default function CompetitorsView() {
  ...
  const getForceGraphOption = () => {
    // Collect all competitor tags and build link relationships
    const baseTags = ['테크', '리뷰', '비교', '가성비', 'IT꿀팁', '언박싱', '내돈내산', '스마트폰', '사용후기', '추천템'];
    
    // Generate nodes
    const nodes = baseTags.map((tag, idx) => {
      // Primary search tag gets larger size
      const isPrimary = idx < 3;
      return {
        id: tag,
        name: tag,
        symbolSize: isPrimary ? 38 : 24,
        value: isPrimary ? 90 : 45,
        category: idx % 3,
        label: {
          show: true,
          position: 'inside',
    ...
    // Generate link edges
    const links = [];
    baseTags.forEach((sourceTag, sIdx) => {
      baseTags.forEach((targetTag, tIdx) => {
        if (sIdx < tIdx && (sIdx + tIdx) % 2 === 0) {
          links.push({
            source: sourceTag,
            target: targetTag,
            lineStyle: {
    ...
    <div className="space-y-6">
      ...
      {subTab === 'matrix' ? (
        ...
        <div className="bg-[#0c0c0f] border border-zinc-800 rounded-xl p-6 shadow-sm">
          ...
          <div className="h-[550px] w-full bg-zinc-950/40 rounded-xl border border-zinc-900">
